...
#ifdef __vita__
    this->context = Ship::Context::CreateUninitializedInstance("Vita Kart 64", "vitakart64", "vitakart64.cfg.json");
#else
...
#ifdef __vita__
    SPDLOG_INFO("Vita Kart 64 " SPAGHETTI_VERSION);
#else
...
void GameEngine::Create() {
    ...
    // Default to 60 FPS on hand-held targets.
    CVarRegisterInteger("gInterpolationFPS", 60);
#endif
}
...
void GameEngine::AudioInit() {
    ...
    if (sequences_files != nullptr) {
        for (auto& sequence : *sequences_files) {
            ...
            if (seq == nullptr || seq->id >= Instance->sequenceTable.size()) {
                continue;
            }
...
extern "C" CtlEntry* GameEngine_LoadBank(const uint8_t bankId) {
    ...
    if (bankId >= engine->banksTable.size()) {
        return nullptr;
    }
