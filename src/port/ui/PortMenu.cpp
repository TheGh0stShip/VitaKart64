...
#ifdef __vita__
constexpr bool ControllerNavDefault = true;
#else
constexpr bool ControllerNavDefault = false;
#endif
...
void PortMenu::AddSettings() {
    ...
    AddWidget(path, "Menu Controller Navigation", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip(
            "Allows controller navigation of the Vita Kart menu (Settings, Enhancements,...)\nCAUTION: "
            "This will disable game inputs while the menu is visible.\n\nD-pad to move between "
            "items, A to select, B to move up in scope.").DefaultValue(ControllerNavDefault));
...
#ifdef __vita__
    const int32_t defaultInterpolationFps = 60;
#else
    const int32_t defaultInterpolationFps = 30;
#endif
    ...
    AddWidget(path, "Current FPS: %d", WIDGET_CVAR_SLIDER_INT)
        ...
        .Options(IntSliderOptions().Tooltip(tooltip).Min(30).Max(maxFps).DefaultValue(defaultInterpolationFps));
...
#ifdef __vita__
    path.sidebarName = "Vita";
    path.column = SECTION_COLUMN_1;
    AddSidebarEntry("Settings", "Vita", 2);
    AddWidget(path, "Vita Kart 64", WIDGET_SEPARATOR_TEXT).Options(WidgetOptions().Color(Colors::Cyan));
    AddWidget(path, "Vita status", WIDGET_TEXT)
        .PreFunc([](WidgetInfo& info) { info.name = VitaPlatform::GetStatusText(); })
        .Options(WidgetOptions().Color(Colors::LightGray));
    AddWidget(path, "Maximum Performance", WIDGET_TEXT)
        .Options(WidgetOptions()
                     .Color(Colors::Cyan)
                     .Tooltip("Always uses the maximum supported 444/222/222/166 MHz clocks and targets 60 FPS."));
    AddWidget(path, "Adaptive Resolution", WIDGET_CVAR_CHECKBOX)
        .CVar(VitaPlatform::AdaptiveResolutionCVar)
        .Callback([](WidgetInfo&) {
            if (!CVarGetInteger(VitaPlatform::AdaptiveResolutionCVar, 1)) {
                VitaPlatform::ApplyMaxPerformance(true);
            }
        })
        .Options(CheckboxOptions()
                     .DefaultValue(true)
                     .Tooltip("Lowers internal resolution in 5% steps after sustained missed 60 FPS deadlines, then "
                              "restores quality after sustained headroom."));
    AddWidget(path, "Adaptive Floor: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar(VitaPlatform::AdaptiveResolutionFloorCVar)
        .Options(FloatSliderOptions()
                     .IsPercentage()
                     .Min(0.60f)
                     .Max(1.00f)
                     ...
                     .Step(0.05f)
                     .ShowButtons(false)
                     .Tooltip("Lowest internal resolution the frame-time governor may select."));
    AddWidget(path, "Telemetry HUD", WIDGET_CVAR_CHECKBOX)
        .CVar(VitaPlatform::TelemetryHudCVar)
        .Options(CheckboxOptions().Tooltip("Shows FPS, frame time, render scale, battery, and temperature."));
    AddWidget(path, "Touch Shortcuts", WIDGET_CVAR_CHECKBOX)
        .CVar(VitaPlatform::TouchShortcutsCVar)
        .Options(CheckboxOptions()
                     .DefaultValue(true)
                     .Tooltip("A two-finger tap toggles the telemetry HUD."));
    AddWidget(path, "Two-finger tap: toggle telemetry HUD", WIDGET_TEXT)
        .Options(WidgetOptions().Color(Colors::Gray));

    path.column = SECTION_COLUMN_2;
    AddWidget(path, "Vita Runtime", WIDGET_SEPARATOR_TEXT).Options(WidgetOptions().Color(Colors::Orange));
    AddWidget(path, "CPU: 444 MHz\nBus: 222 MHz\nGPU: 222 MHz\nGPU Xbar: 166 MHz", WIDGET_TEXT)
        .Options(WidgetOptions().Color(Colors::LightGray));
    AddWidget(path, "60 FPS presentation uses frame interpolation; original game logic remains authentic at 30 Hz.",
              WIDGET_TEXT)
        .Options(WidgetOptions().Color(Colors::Gray));
    AddWidget(path, "Reapply Maximum Performance", WIDGET_BUTTON)
        .Callback([](WidgetInfo&) { VitaPlatform::ApplyMaxPerformance(true); })
        .Options(ButtonOptions().Tooltip("Restores maximum clocks, 60 FPS, VSync, and the default render scale."));
#endif
...
void PortMenu::AddEnhancements() {
    ...
    AddWidget(path, "Show port version", WIDGET_CVAR_CHECKBOX)
        .CVar("gShowSpaghettiVersion")
        .Options(CheckboxOptions().Tooltip("Show the Vita Kart 64 version on the Mario Kart menu").DefaultValue(true));
