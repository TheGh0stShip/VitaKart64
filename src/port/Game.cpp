...
void CM_ThrowRuntimeError(const char* fmt, ...) {
    ...
    const char* crash_desc = "\nVita Kart 64 has crashed. The log in ux0:data/vitakart64 can help diagnose it.";
...
extern "C" void *vita_main(void *argv) {
    ...
    GameEngine::Create();
#ifdef __vita__
    VitaPlatform::Initialize();
    VitaLoadingScreen::SetStage("INITIALIZING AUDIO", "STARTING LOW LATENCY STEREO OUTPUT", 835);
#endif
    audio_init();
    sound_init();

#ifdef __vita__
    VitaLoadingScreen::PrefetchFile("app0:/spaghetti.o2r", "ENGINE ASSET ARCHIVE", 855, 895);
    VitaLoadingScreen::PrefetchFile("ux0:data/vitakart64/mk64.o2r", "MARIO KART ASSET ARCHIVE", 895, 945);
    VitaLoadingScreen::SetStage("INITIALIZING GAME", "PREPARING MENUS RACE DATA AND CONTROLLERS", 955);
#endif
    CustomEngineInit();

#ifdef __vita__
    VitaPreload::WarmRuntimeResources();
    VitaLoadingScreen::End();
#endif

#ifdef __vita__
    CVarSetInteger("gSkipIntro", 1);
