...
#ifdef __vita__
constexpr int ControllerNavDefault = 1;
#else
constexpr int ControllerNavDefault = 0;
#endif

    void SpaghettiGui::DrawMenu() {
        const std::shared_ptr<Window> wnd = Context::GetInstance()->GetWindow();
#ifdef __vita__
        // The Vita only needs a full-screen game surface. Desktop docking and
        // editor windows are prohibitively expensive when rebuilt at 60 Hz.
        const ImGuiViewport* mainViewport = ImGui::GetMainViewport();
        mTemporaryWindowPos = mainViewport->WorkPos;
        ImGui::SetNextWindowPos(mainViewport->WorkPos, ImGuiCond_Always);
        ImGui::SetNextWindowSize(ImVec2(static_cast<float>(wnd->GetWidth()),
                                       static_cast<float>(wnd->GetHeight())), ImGuiCond_Always);
        ...
        if (ImGui::IsKeyPressed(TOGGLE_BTN) || ImGui::IsKeyPressed(ImGuiKey_Escape) ||
            (ImGui::IsKeyPressed(TOGGLE_PAD_BTN) &&
             CVarGetInteger(CVAR_IMGUI_CONTROLLER_NAV, ControllerNavDefault))) {
            ...
            if (CVarGetInteger(CVAR_IMGUI_CONTROLLER_NAV, ControllerNavDefault) && GetMenuOrMenubarVisible()) {
