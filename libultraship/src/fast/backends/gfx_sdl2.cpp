...
extern "C" void vglSetShaderCachePath(const char* path);
extern "C" void vglWaitVblankStart(GLboolean enable);

static void VitaSdlTrace(const char* fmt, ...) {
...
void GfxWindowBackendSDL2::Init(const char* gameName, const char* gfxApiName, bool startFullScreen, uint32_t width,
...
#ifdef __vita__
    VitaSdlTrace("SDL2 Init begin game=%s api=%s requested=%ux%u\n", gameName, gfxApiName, width, height);
    width = 960;
    height = 544;
    posX = 0;
    posY = 0;
...
#ifdef __vita__
    VitaSdlTrace("SDL_Init begin\n");
#endif
    SDL_Init(SDL_INIT_VIDEO);
#ifdef __vita__
    VitaSdlTrace("SDL_Init end err=%s\n", SDL_GetError());
#endif
    ...
    if (use_opengl) {
#ifdef __vita__
        VitaSdlTrace("SDL_GL_SetAttribute skipped for direct vitaGL\n");
#else
#ifdef __vita__
        VitaSdlTrace("SDL_GL_SetAttribute begin depth/stencil/doublebuffer\n");
#endif
        SDL_GL_SetAttribute(SDL_GL_DEPTH_SIZE, 24);
        SDL_GL_SetAttribute(SDL_GL_STENCIL_SIZE, 8);
        SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1);
#ifdef __vita__
        VitaSdlTrace("SDL_GL_SetAttribute end err=%s\n", SDL_GetError());
#endif
#endif
    } else {
...
#ifdef __vita__
    VitaSdlTrace("SDL_CreateWindow begin title=%s pos=%d,%d size=%ux%u flags=0x%x\n", title, posX, posY, mWindowWidth,
                 mWindowHeight, flags);
#endif
    mWnd = SDL_CreateWindow(title, posX, posY, mWindowWidth, mWindowHeight, flags);
#ifdef __vita__
    VitaSdlTrace("SDL_CreateWindow end window=%p err=%s\n", mWnd, SDL_GetError());
#endif
#ifdef _WIN32
...
#ifdef __vita__
        VitaLoadingScreen::Begin();
        sceIoMkdir("ux0:data/vitakart64", 0777);
        sceIoMkdir("ux0:data/vitakart64/shader_cache", 0777);
        sceIoMkdir("ux0:data/vitakart64/shader_cache/v1", 0777);
        sceIoMkdir("ux0:data/vitakart64/shader_cache/v1/v", 0777);
        sceIoMkdir("ux0:data/vitakart64/shader_cache/v1/f", 0777);
        vglSetShaderCachePath("ux0:data/vitakart64/shader_cache");
        VitaSdlTrace("vglInit begin pool=%u window=%p\n", 12U * 1024U * 1024U, mWnd);
        GLboolean vgl_result = vglInit(12 * 1024 * 1024);
        VitaSdlTrace("vglInit end result=%d\n", (int)vgl_result);
        vglWaitVblankStart(GL_TRUE);
        glViewport(0, 0, 960, 544);
        mCtx = reinterpret_cast<SDL_GLContext>(1);
        VitaLoadingScreen::GraphicsReady();
#else
...
#ifdef __vita__
    VitaSdlTrace("Gui Init begin\n");
#endif
    Ship::Context::GetInstance()->GetWindow()->GetGui()->Init(window_impl);
#ifdef __vita__
    VitaSdlTrace("Gui Init end\n");
    VitaLoadingScreen::SetStage("RENDERER READY", "LOADING GAME SYSTEMS AND GRAPHICS PIPELINES", 360);
#endif
...
#ifdef __vita__
    VitaPlatform::HandleEvent(event);
#endif
...
#ifdef __vita__
        // Vita Kart has one release profile: synchronized 60 Hz presentation.
        mVsyncEnabled = true;
        vglWaitVblankStart(GL_TRUE);
#else
...
#ifdef __vita__
    VitaSdlTrace("vglSwapBuffers begin\n");
    vglSwapBuffers(GL_FALSE);
    VitaSdlTrace("vglSwapBuffers end\n");
#else
    SDL_GL_SwapWindow(mWnd);
#endif
#ifdef __vita__
    VitaPlatform::OnFramePresented();
#endif
}
