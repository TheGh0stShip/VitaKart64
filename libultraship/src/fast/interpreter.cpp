...
#ifdef __vita__
#define TEXTURE_CACHE_MAX_SIZE 24576
#else
...
#ifdef __vita__
#include <vitasdk.h>
#include "port/VitaLoadingScreen.h"
typedef enum {
...
#ifdef __vita__
static void VitaRecordShaderProgram(uint64_t id0, uint64_t id1);
static void VitaPrewarmShaders(Interpreter* gfx, uint32_t budget);
#endif
...
#ifdef __vita__
        if (prg != nullptr) {
            VitaRecordShaderProgram(id0, id1);
        }
#endif
...
#ifdef __vita__
bool Interpreter::PreloadTextureResource(const std::shared_ptr<Fast::Texture>& texture) {
    if (texture == nullptr || texture->ImageData == nullptr || texture->Width == 0 || texture->Height == 0) {
        return false;
    }

    if (mTextureCache.map.empty()) {
        mTextureCache.map.reserve(TEXTURE_CACHE_MAX_SIZE);
        mRapi->ReserveTextureSlots(TEXTURE_CACHE_MAX_SIZE);
    }
    ...
    switch (texture->Type) {
        ...
        case TextureType::RGBA16bpp:
            fmt = G_IM_FMT_RGBA;
            siz = G_IM_SIZ_16b;
            rowBytes = texture->Width * 2;
            break;
        case TextureType::Grayscale4bpp:
            fmt = G_IM_FMT_I;
            siz = G_IM_SIZ_4b;
            rowBytes = (texture->Width + 1) / 2;
            break;
        case TextureType::Grayscale8bpp:
            fmt = G_IM_FMT_I;
            siz = G_IM_SIZ_8b;
            rowBytes = texture->Width;
            break;
    ...
    const uint64_t totalBytes64 = static_cast<uint64_t>(rowBytes) * texture->Height;
    if (totalBytes64 == 0 || totalBytes64 > texture->ImageDataSize || totalBytes64 > UINT32_MAX) {
        return false;
    }
    const uint32_t totalBytes = static_cast<uint32_t>(totalBytes64);

    TextureCacheKey key{};
    key.texture_addr = texture->ImageData;
    key.fmt = fmt;
    key.siz = siz;
    key.palette_index = 0;
    key.size_bytes = totalBytes;
    if (TextureCacheLookup(0, key)) {
        mRenderingState.mTextures[0] = nullptr;
        return true;
    }

    const RDP savedRdp = *mRdp;
    RawTexMetadata metadata{};
    metadata.width = texture->Width;
    metadata.height = texture->Height;
    metadata.h_byte_scale = texture->HByteScale;
    metadata.v_pixel_scale = texture->VPixelScale;
    metadata.resource = texture;

    auto& loaded = mRdp->loaded_texture[0];
    loaded.addr = texture->ImageData;
    loaded.orig_size_bytes = totalBytes;
    loaded.size_bytes = totalBytes;
    loaded.full_image_line_size_bytes = rowBytes;
    loaded.line_size_bytes = rowBytes;
    loaded.tex_flags = texture->Flags;
    loaded.raw_tex_metadata = metadata;
    loaded.masked = false;
    loaded.blended = false;

    auto& tile = mRdp->texture_tile[0];
    tile.fmt = fmt;
    tile.siz = siz;
    tile.cms = G_TX_CLAMP;
    tile.cmt = G_TX_CLAMP;
    ...
    tile.uls = 0;
    tile.ult = 0;
    tile.lrs = static_cast<float>((texture->Width - 1) << G_TEXTURE_IMAGE_FRAC);
    tile.lrt = static_cast<float>((texture->Height - 1) << G_TEXTURE_IMAGE_FRAC);
    tile.tmem = 0;
    tile.line_size_bytes = rowBytes;
    tile.palette = 0;
    tile.tmem_index = 0;

    bool uploaded = true;
    if (texture->Type == TextureType::RGBA16bpp) {
        static std::vector<uint16_t> nativePixels;
        const size_t pixelCount = static_cast<size_t>(texture->Width) * texture->Height;
        nativePixels.resize(pixelCount);
        for (size_t i = 0; i < pixelCount; i++) {
            nativePixels[i] = static_cast<uint16_t>((texture->ImageData[i * 2] << 8) | texture->ImageData[i * 2 + 1]);
        }
        uploaded = mRapi->UploadTextureRgba16(nativePixels.data(), texture->Width, texture->Height);
        if (!uploaded) {
            ImportTextureRgba16(0, false);
            uploaded = true;
        }
    } else {
        switch (texture->Type) {
            case TextureType::RGBA32bpp:
                ImportTextureRgba32(0, false);
                break;
            case TextureType::Grayscale4bpp:
                ImportTextureI4(0, false);
                break;
            case TextureType::Grayscale8bpp:
                ImportTextureI8(0, false);
                break;
            case TextureType::GrayscaleAlpha4bpp:
                ImportTextureIA4(0, false);
                break;
            case TextureType::GrayscaleAlpha8bpp:
                ImportTextureIA8(0, false);
                break;
            case TextureType::GrayscaleAlpha16bpp:
                ImportTextureIA16(0, false);
                break;
            default:
                uploaded = false;
                break;
...
bool Interpreter::PreloadTextureResourceWithPalette(const std::shared_ptr<Fast::Texture>& texture,
                                                    const std::shared_ptr<Fast::Texture>& palette,
                                                    uint8_t paletteIndex) {
    if (texture == nullptr || palette == nullptr || texture->ImageData == nullptr || palette->ImageData == nullptr ||
    ...
    key.fmt = G_IM_FMT_CI;
    key.siz = siz;
    key.palette_index = paletteIndex;
    key.size_bytes = totalBytes;

    if (texture->Type == TextureType::Palette4bpp) {
        const uint8_t paletteSlot = paletteIndex / 8;
        if (paletteSlot > 1) {
            return false;
        }
        const uint32_t fullPaletteOffset = paletteSlot * 256U;
        if (palette->ImageDataSize >= fullPaletteOffset + 256U) {
            key.palette_addrs[paletteSlot] = palette->ImageData + fullPaletteOffset;
        } else if (palette->ImageDataSize >= 32U) {
            key.palette_addrs[paletteSlot] = palette->ImageData;
        } else {
            return false;
        }
    } else {
        if (palette->ImageDataSize >= 512U) {
            key.palette_addrs[0] = palette->ImageData;
            key.palette_addrs[1] = palette->ImageData + 256U;
        } else if (palette->ImageDataSize > 0U) {
            key.palette_addrs[0] = palette->ImageData;
    ...
    if (texture->Type == TextureType::Palette4bpp) {
        const uint8_t paletteSlot = paletteIndex / 8;
        const uint32_t paletteOffset = (paletteIndex % 8) * 16U * 2U;
        memset(mRdp->palette_staging[paletteSlot], 0, sizeof(mRdp->palette_staging[paletteSlot]));
        if (palette->ImageDataSize >= paletteSlot * 256U + 256U) {
            memcpy(mRdp->palette_staging[paletteSlot], palette->ImageData + paletteSlot * 256U, 256U);
            mRdp->palette_dram_addr[paletteSlot] = palette->ImageData + paletteSlot * 256U;
        } else {
            memcpy(mRdp->palette_staging[paletteSlot] + paletteOffset, palette->ImageData, 32U);
            mRdp->palette_dram_addr[paletteSlot] = palette->ImageData;
        }
        mRdp->palettes[paletteSlot] = mRdp->palette_staging[paletteSlot];
    } else if (palette->ImageDataSize >= 512U) {
        mRdp->palettes[0] = palette->ImageData;
        mRdp->palettes[1] = palette->ImageData + 256U;
        mRdp->palette_dram_addr[0] = palette->ImageData;
        mRdp->palette_dram_addr[1] = palette->ImageData + 256U;
    } else {
        memset(mRdp->palette_staging[0], 0, sizeof(mRdp->palette_staging[0]));
        memset(mRdp->palette_staging[1], 0, sizeof(mRdp->palette_staging[1]));
        memcpy(mRdp->palette_staging[0], palette->ImageData, std::min<uint32_t>(palette->ImageDataSize, 256U));
    ...
    if (texture->Type == TextureType::Palette4bpp) {
        ImportTextureCi4(0, false);
    } else {
        ImportTextureCi8(0, false);
    }
...
#ifdef __vita__
    {
        uint16_t* nativePixels = reinterpret_cast<uint16_t*>(mTexUploadBuffer);
        uint32_t nativeIndex = 0;

        for (uint32_t y = 0; y < height; y++) {
            for (uint32_t x = 0; x < width; x++) {
                uint32_t clrIdx = (y * (fullImageLineSizeBytes / 2)) + x;
                nativePixels[nativeIndex++] = static_cast<uint16_t>((addr[2 * clrIdx] << 8) | addr[2 * clrIdx + 1]);
            }
        }

        if (mRapi->UploadTextureRgba16(nativePixels, width, height)) {
            return;
        }
    }
#endif
...
#ifdef __vita__
    {
        ...
        for (uint32_t y = 0; y < height; y++) {
            for (uint32_t x = 0; x < width; x++) {
                ...
                nativePixels[nativeIndex++] = static_cast<uint16_t>((palette[idx * 2] << 8) | palette[idx * 2 + 1]);
            }
        }
...
void Interpreter::ImportTextureCi8(int tile, bool importReplacement) {
    ...
    if (mRdp->palettes[0] == nullptr) {
        SPDLOG_WARN("CI8: null low palette (pal0={}, pal1={})", static_cast<const void*>(mRdp->palettes[0]),
...
#ifdef __vita__
    {
        ...
        for (uint32_t i = 0, j = 0; i < sizeBytes; j += fullImageLineSizeBytes - lineSizeBytes) {
            for (uint32_t k = 0; k < lineSizeBytes; i++, k++, j++) {
                uint8_t idx = addr[j];
                const uint8_t* palette = mRdp->palettes[idx / 128];
                nativePixels[i] =
                    palette == nullptr ? 0
                                       : static_cast<uint16_t>((palette[(idx % 128) * 2] << 8) |
                                                               palette[(idx % 128) * 2 + 1]);
            }
        }
    ...
    for (uint32_t i = 0, j = 0; i < sizeBytes; j += fullImageLineSizeBytes - lineSizeBytes) {
        for (uint32_t k = 0; k < lineSizeBytes; i++, k++, j++) {
            ...
            const uint8_t* palette = mRdp->palettes[idx / 128];
            uint16_t col16 =
                palette == nullptr ? 0 : (palette[(idx % 128) * 2] << 8) | palette[(idx % 128) * 2 + 1];
            uint8_t a = col16 & 1;
            uint8_t r = col16 >> 11;
            uint8_t g = (col16 >> 6) & 0x1f;
            uint8_t b = (col16 >> 1) & 0x1f;
...
#if defined(__vita__) && VITAKART_TEXTURE_MISS_LOG
struct VitaPaletteSource {
    const uint8_t* addr = nullptr;
    std::string path;
};

static VitaPaletteSource sVitaPaletteSources[8];
static uint32_t sVitaPaletteSourceIndex = 0;

static void VitaRememberPaletteSource(const uint8_t* addr, const RawTexMetadata* metadata) {
    if (addr == nullptr || metadata == nullptr || metadata->resource == nullptr || metadata->resource->GetInitData() == nullptr) {
        return;
    }

    for (VitaPaletteSource& source : sVitaPaletteSources) {
        if (source.addr == addr) {
            source.path = metadata->resource->GetInitData()->Path;
            return;
        }
    }

    VitaPaletteSource& source = sVitaPaletteSources[sVitaPaletteSourceIndex++ % 8];
    source.addr = addr;
    source.path = metadata->resource->GetInitData()->Path;
}

static const char* VitaFindPalettePath(const uint8_t* addr) {
    if (addr == nullptr) {
        return "";
    }

    for (const VitaPaletteSource& source : sVitaPaletteSources) {
        if (source.addr == addr && !source.path.empty()) {
...
static void VitaRecordTextureMiss(const RawTexMetadata* metadata, uint8_t fmt, uint8_t siz, uint8_t paletteIndex,
                                  uint32_t origSizeBytes, const uint8_t* textureAddr, const uint8_t* paletteAddr0,
                                  const uint8_t* paletteAddr1) {
    sceIoMkdir("ux0:data", 0777);
    sceIoMkdir("ux0:data/vitakart64", 0777);

    const char* resourcePath = "<raw>";
    if (metadata != nullptr && metadata->resource != nullptr && metadata->resource->GetInitData() != nullptr) {
        resourcePath = metadata->resource->GetInitData()->Path.c_str();
    }

    FILE* misses = fopen("ux0:data/vitakart64/texture-misses.log", "a");
    if (misses == nullptr) {
        return;
    }

    fprintf(misses,
            "resource=\"%s\" palette_resource0=\"%s\" palette_resource1=\"%s\" fmt=%u siz=%u palette=%u bytes=%u "
            "texture=%p palette0=%p palette1=%p\n",
            resourcePath, VitaFindPalettePath(paletteAddr0), VitaFindPalettePath(paletteAddr1),
            static_cast<unsigned>(fmt), static_cast<unsigned>(siz), static_cast<unsigned>(paletteIndex),
            static_cast<unsigned>(origSizeBytes), static_cast<const void*>(textureAddr),
            static_cast<const void*>(paletteAddr0), static_cast<const void*>(paletteAddr1));
    fclose(misses);
}
#endif
...
#if defined(__vita__) && VITAKART_TEXTURE_MISS_LOG
    VitaRecordTextureMiss(metadata, fmt, siz, paletteIndex, origSizeBytes, origAddr, key.palette_addrs[0],
                          key.palette_addrs[1]);
#endif
...
void Interpreter::GfxSpVertex(size_t n_vertices, size_t dest_index, const F3DVtx* vertices) {
	...
	float xAdjust = AdjXForAspectRatio(1.f);
...
void Interpreter::GfxSpTri1(uint8_t vtx1_idx, uint8_t vtx2_idx, uint8_t vtx3_idx, bool is_rect) {
    ...
    if (!mShaderStack.empty()) {
        cc_options |= (static_cast<uint64_t>(mShaderStack.top()) << SHADER_ID_SHIFT);
    } else {
        cc_options |= ~0ULL << SHADER_ID_SHIFT;
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        *buf_vbo_ptr++ = invertY ? -v_arr[i]->y : v_arr[i]->y;
        *buf_vbo_ptr++ = z;
        *buf_vbo_ptr++ = w;
        ...
        if (use_fog) {
            if (use_blend_color) {
                ...
                *buf_vbo_ptr++ = mRdp->blend_color.r * (1.0f / 255.0f);
                *buf_vbo_ptr++ = mRdp->blend_color.g * (1.0f / 255.0f);
                *buf_vbo_ptr++ = mRdp->blend_color.b * (1.0f / 255.0f);
                *buf_vbo_ptr++ = mRdp->fog_color.a * (1.0f / 255.0f);
            } else {
                *buf_vbo_ptr++ = mRdp->fog_color.r * (1.0f / 255.0f);
                *buf_vbo_ptr++ = mRdp->fog_color.g * (1.0f / 255.0f);
                *buf_vbo_ptr++ = mRdp->fog_color.b * (1.0f / 255.0f);
                *buf_vbo_ptr++ = v_arr[i]->color.a * (1.0f / 255.0f); // fog factor (not alpha)
            }
        }

        if (use_grayscale) {
            *buf_vbo_ptr++ = mRdp->grayscale_color.r * (1.0f / 255.0f);
            *buf_vbo_ptr++ = mRdp->grayscale_color.g * (1.0f / 255.0f);
            *buf_vbo_ptr++ = mRdp->grayscale_color.b * (1.0f / 255.0f);
            *buf_vbo_ptr++ = mRdp->grayscale_color.a * (1.0f / 255.0f); // lerp interpolation factor (not alpha)
        }
...
void Interpreter::GfxDpLoadTlut(uint8_t tile, uint32_t high_index) {
    ...
    if (tmem >= 256) {
        ...
        if (high_index == 255 && paletteByteOffset == 0) {
...
#if defined(__vita__) && VITAKART_TEXTURE_MISS_LOG
            VitaRememberPaletteSource(mRdp->palette_dram_addr[0], &mRdp->texture_to_load.raw_tex_metadata);
            VitaRememberPaletteSource(mRdp->palette_dram_addr[1], &mRdp->texture_to_load.raw_tex_metadata);
#endif
            ...
            mRdp->palette_dram_addr[0] = src;
#ifdef __vita__
            if (paletteByteOffset == 0 && byteCount < 256 &&
...
#ifdef __vita__
struct VitaWarmupCombine {
    uint64_t combineMode;
    bool twoCycle;
};

static constexpr uint32_t VitaColorComb(uint32_t a, uint32_t b, uint32_t c, uint32_t d) {
    return (a & 0xf) | ((b & 0xf) << 4) | ((c & 0x1f) << 8) | ((d & 7) << 13);
}

static constexpr uint32_t VitaAlphaComb(uint32_t a, uint32_t b, uint32_t c, uint32_t d) {
    return (a & 7) | ((b & 7) << 3) | ((c & 7) << 6) | ((d & 7) << 9);
}

static constexpr uint64_t VitaCombineMode(uint32_t rgb, uint32_t alpha, uint32_t rgbCyc2, uint32_t alphaCyc2) {
    return rgb | (static_cast<uint64_t>(alpha) << 16) | (static_cast<uint64_t>(rgbCyc2) << 28) |
           (static_cast<uint64_t>(alphaCyc2) << 44);
}

#define VITA_RGB(a, b, c, d) VitaColorComb(G_CCMUX_##a, G_CCMUX_##b, G_CCMUX_##c, G_CCMUX_##d)
#define VITA_ALPHA(a, b, c, d) VitaAlphaComb(G_ACMUX_##a, G_ACMUX_##b, G_ACMUX_##c, G_ACMUX_##d)
#define VITA_WARMUP_LERP(a0, b0, c0, d0, Aa0, Ab0, Ac0, Ad0, a1, b1, c1, d1, Aa1, Ab1, Ac1, Ad1, cyc2) \
    {                                                                                                      \
        VitaCombineMode(VITA_RGB(a0, b0, c0, d0), VITA_ALPHA(Aa0, Ab0, Ac0, Ad0),                         \
                        VITA_RGB(a1, b1, c1, d1), VITA_ALPHA(Aa1, Ab1, Ac1, Ad1)),                        \
            cyc2                                                                                          \
    }

struct VitaShaderProgramId {
    uint64_t id0;
    uint64_t id1;
};

static bool operator==(const VitaShaderProgramId& lhs, const VitaShaderProgramId& rhs) {
    return lhs.id0 == rhs.id0 && lhs.id1 == rhs.id1;
}

struct VitaShaderProgramIdHash {
    size_t operator()(const VitaShaderProgramId& value) const {
        const uint64_t mixed = value.id0 ^ (value.id1 + 0x9e3779b97f4a7c15ULL + (value.id0 << 6) + (value.id0 >> 2));
        return static_cast<size_t>(mixed ^ (mixed >> 32));
    }
};

static constexpr uint32_t kVitaShaderManifestMax = 8192;
static constexpr const char* kVitaShaderSeedPath = "app0:/shader-manifest.txt";
static constexpr const char* kVitaShaderManifestPath = "ux0:data/vitakart64/shader-manifest.txt";
static constexpr const char* kVitaShaderWarmupSummaryPath = "ux0:data/vitakart64/shader-warmup-summary.txt";
static VitaShaderProgramId sVitaShaderManifest[kVitaShaderManifestMax];
static std::unordered_set<VitaShaderProgramId, VitaShaderProgramIdHash> sVitaShaderManifestSet;
static uint32_t sVitaShaderManifestCount = 0;
static uint32_t sVitaShaderManifestWarmupIndex = 0;
static uint32_t sVitaShaderWarmupCompiledCount = 0;
static uint32_t sVitaShaderWarmupResidentCount = 0;
static bool sVitaShaderManifestLoaded = false;
static bool sVitaShaderPresetWarmupComplete = false;
static bool sVitaShaderWarmupCompiled = false;
static bool sVitaShaderWarmupActive = false;
static uint32_t sVitaShaderWarmupBudget = 0;

static void VitaEnsureShaderManifestDir() {
    sceIoMkdir("ux0:data", 0777);
    sceIoMkdir("ux0:data/vitakart64", 0777);
}

static bool VitaShaderManifestContains(uint64_t id0, uint64_t id1) {
    return sVitaShaderManifestSet.find({ id0, id1 }) != sVitaShaderManifestSet.end();
}

static void VitaLoadShaderManifestFile(const char* path) {
    ...
    while (sVitaShaderManifestCount < kVitaShaderManifestMax &&
           fscanf(manifest, "%llx %llx", &id0, &id1) == 2) {
        const uint64_t shaderId0 = static_cast<uint64_t>(id0);
        const uint64_t shaderId1 = static_cast<uint64_t>(id1);
        const VitaShaderProgramId shader = { shaderId0, shaderId1 };
        if (sVitaShaderManifestSet.insert(shader).second) {
            sVitaShaderManifest[sVitaShaderManifestCount++] = shader;
        }
    }
...
static void VitaLoadShaderManifest() {
    if (sVitaShaderManifestLoaded) {
        return;
    }

    sVitaShaderManifestLoaded = true;
    VitaEnsureShaderManifestDir();
    sVitaShaderManifestSet.reserve(kVitaShaderManifestMax);
    VitaLoadShaderManifestFile(kVitaShaderSeedPath);
    VitaLoadShaderManifestFile(kVitaShaderManifestPath);
}

static void VitaRecordShaderProgram(uint64_t id0, uint64_t id1) {
    if (sVitaShaderWarmupActive) {
        return;
    }

    VitaLoadShaderManifest();
    if (VitaShaderManifestContains(id0, id1) || sVitaShaderManifestCount >= kVitaShaderManifestMax) {
        return;
    }

    const VitaShaderProgramId shader = { id0, id1 };
    sVitaShaderManifestSet.insert(shader);
    sVitaShaderManifest[sVitaShaderManifestCount++] = shader;

    FILE* manifest = fopen(kVitaShaderManifestPath, "a");
    if (manifest == nullptr) {
        return;
    }

    fprintf(manifest, "%016llx %016llx\n", static_cast<unsigned long long>(id0),
            static_cast<unsigned long long>(id1));
    fclose(manifest);
}

static ShaderProgram* VitaPrewarmShaderId(Interpreter* gfx, uint64_t id0, uint64_t id1) {
    if (sVitaShaderWarmupBudget == 0) {
        return nullptr;
    }

    if (ShaderProgram* resident = gfx->mRapi->LookupShader(id0, id1); resident != nullptr) {
        sVitaShaderWarmupResidentCount++;
        return resident;
    }

    sVitaShaderWarmupActive = true;
    ShaderProgram* prg = gfx->LookupOrCreateShaderProgram(id0, id1);
    sVitaShaderWarmupActive = false;

    if (prg != nullptr) {
        sVitaShaderWarmupBudget--;
        sVitaShaderWarmupCompiled = true;
        sVitaShaderWarmupCompiledCount++;
    }
    return prg;
}

static void VitaWriteShaderWarmupSummary(uint32_t presetTotal) {
    VitaEnsureShaderManifestDir();
    FILE* summary = fopen(kVitaShaderWarmupSummaryPath, "w");
    if (summary == nullptr) {
        return;
    }

    fprintf(summary, "schema=vitakart64.shader-warmup.v1\n");
    fprintf(summary, "shader_manifest_entries=%u\n", sVitaShaderManifestCount);
    fprintf(summary, "shader_manifest_warmup_index=%u\n", sVitaShaderManifestWarmupIndex);
    fprintf(summary, "shader_manifest_capacity=%u\n", kVitaShaderManifestMax);
    fprintf(summary, "shader_manifest_capacity_full=%u\n",
            sVitaShaderManifestCount >= kVitaShaderManifestMax ? 1U : 0U);
    fprintf(summary, "shader_preset_total=%u\n", presetTotal);
    fprintf(summary, "shader_preset_warmup_complete=%u\n", sVitaShaderPresetWarmupComplete ? 1U : 0U);
    fprintf(summary, "shader_warmup_compiled_any=%u\n", sVitaShaderWarmupCompiled ? 1U : 0U);
    fprintf(summary, "shader_warmup_compiled_count=%u\n", sVitaShaderWarmupCompiledCount);
    fprintf(summary, "shader_warmup_resident_count=%u\n", sVitaShaderWarmupResidentCount);
    fclose(summary);
}

static void VitaPrewarmShader(Interpreter* gfx, uint64_t combineMode, uint64_t options, uint64_t shaderId1Or = 0) {
    ColorCombinerKey key{};
    key.combine_mode = combineMode;
    key.options = options;
    key.shader_id = 0;

    auto combinerIt = gfx->mColorCombinerPool.find(key);
    if (combinerIt == gfx->mColorCombinerPool.end()) {
        combinerIt = gfx->mColorCombinerPool.insert(std::make_pair(key, ColorCombiner())).first;
        Interpreter::GenerateCC(&combinerIt->second, key);
    }
    ColorCombiner* cachedCombiner = &combinerIt->second;

    ShaderProgram* prg = VitaPrewarmShaderId(gfx, cachedCombiner->shader_id0, cachedCombiner->shader_id1 | shaderId1Or);
    if (prg != nullptr) {
        uint32_t textureMode = 0;
        textureMode |= (shaderId1Or & SHADER_OPT(TEXEL0_CLAMP_S)) ? 1U : 0U;
        textureMode |= (shaderId1Or & SHADER_OPT(TEXEL0_CLAMP_T)) ? 2U : 0U;
        textureMode |= (shaderId1Or & SHADER_OPT(TEXEL1_CLAMP_S)) ? 4U : 0U;
        textureMode |= (shaderId1Or & SHADER_OPT(TEXEL1_CLAMP_T)) ? 8U : 0U;
        cachedCombiner->prg[textureMode] = prg;
    }
}
...
static void VitaPrewarmShaders(Interpreter* gfx, uint32_t budget) {
    ...
    VitaLoadShaderManifest();
    sVitaShaderWarmupBudget = budget;
    sVitaShaderWarmupCompiled = false;
    sVitaShaderWarmupCompiledCount = 0;
    sVitaShaderWarmupResidentCount = 0;

    while (sVitaShaderWarmupBudget > 0 && sVitaShaderManifestWarmupIndex < sVitaShaderManifestCount) {
        const VitaShaderProgramId shader = sVitaShaderManifest[sVitaShaderManifestWarmupIndex++];
        VitaPrewarmShaderId(gfx, shader.id0, shader.id1);
        VitaLoadingScreen::SetProgress("PRECOMPILING GRAPHICS PIPELINES", "HARDWARE OBSERVED SHADER",
                                       sVitaShaderManifestWarmupIndex, sVitaShaderManifestCount, 380, 560);
    }

    static constexpr uint64_t kNoShaderStack = ~0ULL << SHADER_ID_SHIFT;
    static constexpr uint64_t kOptionProfiles[] = {
        0,
        SHADER_OPT(ALPHA),
        SHADER_OPT(FOG),
        SHADER_OPT(ALPHA) | SHADER_OPT(FOG),
        SHADER_OPT(ALPHA) | SHADER_OPT(TEXTURE_EDGE),
        SHADER_OPT(ALPHA) | SHADER_OPT(ALPHA_THRESHOLD),
        SHADER_OPT(ALPHA) | SHADER_OPT(FOG) | SHADER_OPT(ALPHA_THRESHOLD),
        SHADER_OPT(NOISE),
        SHADER_OPT(GRAYSCALE),
        SHADER_OPT(ALPHA) | SHADER_OPT(TEXEL0_MASK),
        SHADER_OPT(ALPHA) | SHADER_OPT(TEXEL0_BLEND),
        SHADER_OPT(ALPHA) | SHADER_OPT(TEXEL0_MASK) | SHADER_OPT(TEXEL1_MASK),
        SHADER_OPT(ALPHA) | SHADER_OPT(TEXEL0_BLEND) | SHADER_OPT(TEXEL1_BLEND),
    };
    static constexpr uint64_t kClampProfiles[] = {
        0 * SHADER_OPT(TEXEL0_CLAMP_S),
        1 * SHADER_OPT(TEXEL0_CLAMP_S),
        2 * SHADER_OPT(TEXEL0_CLAMP_S),
        3 * SHADER_OPT(TEXEL0_CLAMP_S),
        4 * SHADER_OPT(TEXEL0_CLAMP_S),
        5 * SHADER_OPT(TEXEL0_CLAMP_S),
        6 * SHADER_OPT(TEXEL0_CLAMP_S),
        7 * SHADER_OPT(TEXEL0_CLAMP_S),
        8 * SHADER_OPT(TEXEL0_CLAMP_S),
        9 * SHADER_OPT(TEXEL0_CLAMP_S),
        10 * SHADER_OPT(TEXEL0_CLAMP_S),
        11 * SHADER_OPT(TEXEL0_CLAMP_S),
        12 * SHADER_OPT(TEXEL0_CLAMP_S),
        13 * SHADER_OPT(TEXEL0_CLAMP_S),
        14 * SHADER_OPT(TEXEL0_CLAMP_S),
        15 * SHADER_OPT(TEXEL0_CLAMP_S),
    };

    static constexpr VitaWarmupCombine kCombines[] = {
        VITA_WARMUP_LERP(0, 0, 0, PRIMITIVE, 0, 0, 0, PRIMITIVE, 0, 0, 0, PRIMITIVE, 0, 0, 0, PRIMITIVE, false),
        VITA_WARMUP_LERP(0, 0, 0, SHADE, 0, 0, 0, SHADE, 0, 0, 0, SHADE, 0, 0, 0, SHADE, false),
        VITA_WARMUP_LERP(TEXEL0, 0, SHADE, 0, 0, 0, 0, SHADE, TEXEL0, 0, SHADE, 0, 0, 0, 0, SHADE, false),
        VITA_WARMUP_LERP(TEXEL0, 0, SHADE, 0, TEXEL0, 0, SHADE, 0, TEXEL0, 0, SHADE, 0, TEXEL0, 0, SHADE, 0, false),
        VITA_WARMUP_LERP(TEXEL0, 0, SHADE, 0, 0, 0, 0, TEXEL0, TEXEL0, 0, SHADE, 0, 0, 0, 0, TEXEL0, false),
        VITA_WARMUP_LERP(TEXEL0, 0, PRIMITIVE, 0, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0, 0, 0, 0, PRIMITIVE, false),
        VITA_WARMUP_LERP(TEXEL0, 0, PRIMITIVE, 0, TEXEL0, 0, PRIMITIVE, 0, TEXEL0, 0, PRIMITIVE, 0, TEXEL0, 0, PRIMITIVE, 0, false),
        VITA_WARMUP_LERP(0, 0, 0, TEXEL0, 0, 0, 0, SHADE, 0, 0, 0, TEXEL0, 0, 0, 0, SHADE, false),
        VITA_WARMUP_LERP(0, 0, 0, TEXEL0, 0, 0, 0, TEXEL0, 0, 0, 0, TEXEL0, 0, 0, 0, TEXEL0, false),
        VITA_WARMUP_LERP(ENVIRONMENT, SHADE, TEXEL0, SHADE, 0, 0, 0, SHADE, ENVIRONMENT, SHADE, TEXEL0, SHADE, 0, 0, 0, SHADE, false),
        VITA_WARMUP_LERP(ENVIRONMENT, SHADE, TEXEL0, SHADE, TEXEL0, 0, SHADE, 0, ENVIRONMENT, SHADE, TEXEL0, SHADE, TEXEL0, 0, SHADE, 0, false),
        VITA_WARMUP_LERP(TEXEL0, SHADE, TEXEL0_ALPHA, SHADE, 0, 0, 0, SHADE, TEXEL0, SHADE, TEXEL0_ALPHA, SHADE, 0, 0, 0, SHADE, false),
        VITA_WARMUP_LERP(ENVIRONMENT, 0, TEXEL0, SHADE, 0, 0, 0, SHADE, ENVIRONMENT, 0, TEXEL0, SHADE, 0, 0, 0, SHADE, false),
        VITA_WARMUP_LERP(PRIMITIVE, SHADE, TEXEL0, SHADE, 0, 0, 0, SHADE, PRIMITIVE, SHADE, TEXEL0, SHADE, 0, 0, 0, SHADE, false),
        VITA_WARMUP_LERP(0, 0, 0, SHADE, 0, 0, 0, TEXEL0, 0, 0, 0, SHADE, 0, 0, 0, TEXEL0, false),
        VITA_WARMUP_LERP(PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, SHADE, 0, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, SHADE, 0, false),
        VITA_WARMUP_LERP(TEXEL1, TEXEL0, PRIM_LOD_FRAC, TEXEL0, TEXEL1, TEXEL0, PRIM_LOD_FRAC, TEXEL0, TEXEL1, TEXEL0, PRIM_LOD_FRAC, TEXEL0, TEXEL1, TEXEL0, PRIM_LOD_FRAC, TEXEL0, false),
        VITA_WARMUP_LERP(TEXEL1, TEXEL0, LOD_FRACTION, TEXEL0, TEXEL1, TEXEL0, LOD_FRACTION, TEXEL0, TEXEL1, TEXEL0, LOD_FRACTION, TEXEL0, TEXEL1, TEXEL0, LOD_FRACTION, TEXEL0, false),
        VITA_WARMUP_LERP(TEXEL0, 0, TEXEL1, 0, TEXEL0, 0, TEXEL1, 0, TEXEL0, 0, TEXEL1, 0, TEXEL0, 0, TEXEL1, 0, false),
        VITA_WARMUP_LERP(TEXEL0, 0, SHADE, 0, 0, 0, 0, SHADE, 0, 0, 0, COMBINED, 0, 0, 0, COMBINED, true),
        VITA_WARMUP_LERP(0, 0, 0, TEXEL0, 0, 0, 0, TEXEL0, 0, 0, 0, COMBINED, 0, 0, 0, COMBINED, true),
        VITA_WARMUP_LERP(TEXEL0, 0, SHADE, 0, 0, 0, 0, 1, COMBINED, 0, PRIMITIVE, 0, 0, 0, 0, COMBINED, true),
        VITA_WARMUP_LERP(TEXEL0, 0, SHADE, 0, 0, 0, 0, TEXEL0, COMBINED, 0, PRIMITIVE, 0, 0, 0, 0, COMBINED, true),
        VITA_WARMUP_LERP(TEXEL0, 0, SHADE, 0, 0, 0, 0, ENVIRONMENT, TEXEL0, 0, SHADE, 0, 0, 0, 0, ENVIRONMENT, true),
        VITA_WARMUP_LERP(0, 0, 0, SHADE, 0, 0, 0, ENVIRONMENT, 0, 0, 0, SHADE, 0, 0, 0, ENVIRONMENT, false),
        VITA_WARMUP_LERP(0, 0, 0, TEXEL0, 0, 0, 0, TEXEL1, 0, 0, 0, COMBINED, 0, 0, 0, COMBINED, true),
        VITA_WARMUP_LERP(1, 0, SHADE, 0, 0, 0, 0, TEXEL0, 1, 0, SHADE, 0, 0, 0, 0, TEXEL0, false),
        VITA_WARMUP_LERP(0, 0, 0, TEXEL0, TEXEL0, 0, PRIMITIVE, 0, 0, 0, 0, TEXEL0, TEXEL0, 0, PRIMITIVE, 0, false),
        VITA_WARMUP_LERP(0, 0, 0, 1, TEXEL0, 0, PRIMITIVE, 0, 0, 0, 0, 1, TEXEL0, 0, PRIMITIVE, 0, false),
        VITA_WARMUP_LERP(0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0, false),
        VITA_WARMUP_LERP(0, 0, 0, SHADE, TEXEL0, 0, PRIMITIVE, 0, 0, 0, 0, SHADE, TEXEL0, 0, PRIMITIVE, 0, false),
        VITA_WARMUP_LERP(1, PRIMITIVE_ALPHA, TEXEL0, PRIMITIVE, 0, 0, 0, TEXEL0, 1, PRIMITIVE_ALPHA, TEXEL0, PRIMITIVE, 0, 0, 0, TEXEL0, false),
        VITA_WARMUP_LERP(1, ENVIRONMENT, TEXEL0, PRIMITIVE, PRIMITIVE, 0, TEXEL0, 0, 1, ENVIRONMENT, TEXEL0, PRIMITIVE, PRIMITIVE, 0, TEXEL0, 0, false),
    };

    static uint32_t sPresetCombineIndex = 0;
    static uint32_t sPresetOptionIndex = 0;
    static uint32_t sPresetClampIndex = 0;
    const uint32_t presetTotal = static_cast<uint32_t>((sizeof(kCombines) / sizeof(kCombines[0])) *
                                                       (sizeof(kOptionProfiles) / sizeof(kOptionProfiles[0])) *
                                                       (sizeof(kClampProfiles) / sizeof(kClampProfiles[0])));

    while (!sVitaShaderPresetWarmupComplete && sVitaShaderWarmupBudget > 0) {
        const VitaWarmupCombine& combine = kCombines[sPresetCombineIndex];
        uint64_t options = kNoShaderStack | kOptionProfiles[sPresetOptionIndex];
        if (combine.twoCycle) {
            options |= SHADER_OPT(_2CYC);
        }
        VitaPrewarmShader(gfx, combine.combineMode, options, kClampProfiles[sPresetClampIndex]);

        sPresetClampIndex++;
        if (sPresetClampIndex >= sizeof(kClampProfiles) / sizeof(kClampProfiles[0])) {
            sPresetClampIndex = 0;
            sPresetOptionIndex++;
        }
        if (sPresetOptionIndex >= sizeof(kOptionProfiles) / sizeof(kOptionProfiles[0])) {
            sPresetOptionIndex = 0;
            sPresetCombineIndex++;
        }
        if (sPresetCombineIndex >= sizeof(kCombines) / sizeof(kCombines[0])) {
            sVitaShaderPresetWarmupComplete = true;
        }
        const uint32_t presetCurrent = std::min(
            presetTotal,
            static_cast<uint32_t>((sPresetCombineIndex * (sizeof(kOptionProfiles) / sizeof(kOptionProfiles[0])) +
                                   sPresetOptionIndex) *
                                      (sizeof(kClampProfiles) / sizeof(kClampProfiles[0])) +
                                  sPresetClampIndex));
        VitaLoadingScreen::SetProgress("PRECOMPILING GRAPHICS PIPELINES", "GAME COVERAGE SHADER",
                                       presetCurrent, presetTotal, 560, 820);
    }

    if (sVitaShaderWarmupCompiled && gfx->mRenderingState.mShaderProgram != nullptr) {
        gfx->mRapi->UnloadShader(gfx->mRenderingState.mShaderProgram);
        gfx->mRenderingState.mShaderProgram = nullptr;
    }
    gfx->mPrevCombiner = gfx->mColorCombinerPool.end();
    VitaWriteShaderWarmupSummary(presetTotal);
}

#undef VITA_WARMUP_LERP
#undef VITA_ALPHA
#undef VITA_RGB
#endif
...
#ifdef __vita__
    mGfxCurrentWindowDimensions.width = width;
    mGfxCurrentWindowDimensions.height = height;
    mGameWindowViewport = { 0, 0, width, height };
    SetResolutionMultiplier(mCurDimensions.internal_mul);
#endif
...
#ifdef __vita__
        int max_tex_size = std::min(1024, mRapi->GetMaxTextureSize());
#else
...
#ifdef __vita__
    VitaPrewarmShaders(this, std::numeric_limits<uint32_t>::max());
#endif
}
...
void Interpreter::StartFrame() {
    mWapi->GetDimensions(&mGfxCurrentWindowDimensions.width, &mGfxCurrentWindowDimensions.height, &mCurWindowPosX,
                         &mCurWindowPosY);
#ifdef __vita__
    mGameWindowViewport = { 0, 0, mGfxCurrentWindowDimensions.width, mGfxCurrentWindowDimensions.height };
    SetResolutionMultiplier(mCurDimensions.internal_mul);
#endif
...
#ifdef __vita__
    // MK64 normally covers the N64 framebuffer itself, but widescreen transition
    // display lists can leave edge pixels untouched. Never preserve stale color.
...
#ifdef __vita__
    const uint32_t baseWidth = mGfxCurrentWindowDimensions.width ? mGfxCurrentWindowDimensions.width : 960;
    const uint32_t baseHeight = mGfxCurrentWindowDimensions.height ? mGfxCurrentWindowDimensions.height : 544;
    mCurDimensions.width = std::max(1u, static_cast<uint32_t>(std::round(baseWidth * multiplier)));
    mCurDimensions.height = std::max(1u, static_cast<uint32_t>(std::round(baseHeight * multiplier)));
#endif
}
