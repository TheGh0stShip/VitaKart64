...
#ifdef __vita__
    uintptr_t vitaFb = Ship::Context::GetInstance()->GetWindow()->GetGfxFrameBuffer();
    if (vitaFb) {
        const ImVec2 displaySize = ImGui::GetIO().DisplaySize;
        auto interpreter = mInterpreter.lock();
        const float renderWidth =
            interpreter && interpreter->mCurDimensions.width > 0 ? static_cast<float>(interpreter->mCurDimensions.width) : 1.0f;
        const float renderHeight =
            interpreter && interpreter->mCurDimensions.height > 0 ? static_cast<float>(interpreter->mCurDimensions.height) : 1.0f;
        const ImVec2 uv0(0.5f / renderWidth, 0.5f / renderHeight);
        const ImVec2 uv1(1.0f - (0.5f / renderWidth), 1.0f - (0.5f / renderHeight));
        const ImVec2 windowPos = ImGui::GetWindowPos();
        ImDrawList* drawList = ImGui::GetWindowDrawList();
        drawList->AddRectFilled(windowPos, ImVec2(windowPos.x + displaySize.x, windowPos.y + displaySize.y),
                                IM_COL32(0, 0, 0, 255));
        drawList->AddImage(reinterpret_cast<ImTextureID>(vitaFb), ImVec2(windowPos.x - 1.0f, windowPos.y - 1.0f),
                           ImVec2(windowPos.x + displaySize.x + 1.0f, windowPos.y + displaySize.y + 1.0f), uv0,
                           uv1);
    }
...
void Gui::StartDraw() {
    ...
    if (interpreter) {
        ...
        uint32_t displayWidth = displaySize.x > 0.0f ? static_cast<uint32_t>(displaySize.x + 0.5f) : 960;
        uint32_t displayHeight = displaySize.y > 0.0f ? static_cast<uint32_t>(displaySize.y + 0.5f) : 544;
        if (displayWidth == 0) {
            displayWidth = 1;
        }
        if (displayHeight == 0) {
        ...
        interpreter->mGameWindowViewport = { 0, 0, static_cast<int16_t>(displayWidth),
                                             static_cast<int16_t>(displayHeight) };

        const float scale = interpreter->mCurDimensions.internal_mul;
        uint32_t renderWidth = static_cast<uint32_t>((displayWidth * scale) + 0.5f);
        uint32_t renderHeight = static_cast<uint32_t>((displayHeight * scale) + 0.5f);
        interpreter->mCurDimensions.width = renderWidth ? renderWidth : 1;
        interpreter->mCurDimensions.height = renderHeight ? renderHeight : 1;
    }
    return;
#endif
