...
#define SHADER_OPT(opt) (1ULL << static_cast<int>(ShaderOpts::opt))
#endif
...
class Interpreter {
  public:
    ...
    bool PreloadTextureResourceWithPalette(const std::shared_ptr<Fast::Texture>& texture,
                                           const std::shared_ptr<Fast::Texture>& palette, uint8_t paletteIndex);
#endif
    ...
    robin_hood::unordered_map<size_t, const char*> mShaders;
