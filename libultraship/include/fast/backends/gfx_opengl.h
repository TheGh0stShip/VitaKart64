...
class GfxRenderingAPIOGL final : public GfxRenderingAPI {
  public:
    ...
    void ReserveTextureSlots(uint32_t count) override;
    uint32_t NewTexture() override;
    void SelectTexture(int tile, uint32_t textureId) override;
    void UploadTexture(const uint8_t* rgba32Buf, uint32_t width, uint32_t height) override;
    bool UploadTextureRgba16(const uint16_t* rgba16Buf, uint32_t width, uint32_t height) override;
    void InvalidateTextureBindings() override;
  ...
  private:
    ...
    std::vector<GLuint> mReservedTextureIds;
    std::unordered_map<GLuint, SamplerStateOGL> mSamplerStates;
    std::unordered_set<GLuint> mGeneratedTextureIds;
    uint32_t mGeneratedTextureSlots = 0;
